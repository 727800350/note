...
\href{http://i.imgbox.com/1adl5otW.png}{$\theta$的积分域从$0$到$\pi/2$的图析}

从图中可以看到, 当$\theta$取一个值时, 实际上得到的是一个c\^one, 因为$\phi$ 会旋转$2\pi$个角度, 同时, 由于只考虑半球面,所以$\theta$ 只用取$0$到$\pi/2$
